...
import { type ModelInfo, type ProviderSettings, openAiModelInfoSaneDefaults } from "@bro-code/types"
import { openAiCodexDefaultModelId, broGatewayDefaultModelId } from "@bro-code/types"
...
describe("ApiOptions", () => {
	...
	it("initializes broGatewayModelId to its default when switching provider to bro-gateway", () => {
		// Regression: bro-gateway was previously missing from PROVIDER_MODEL_CONFIG, so switching
		// providers never seeded broGatewayModelId. Configs were left without a model id, which
		...
		const providerSelectContainer = screen.getByTestId("provider-select")
		const providerSelect = providerSelectContainer.querySelector("select") as HTMLSelectElement
		fireEvent.change(providerSelect, { target: { value: "bro-gateway" } })

		expect(mockSetApiConfigurationField).toHaveBeenCalledWith("apiProvider", "bro-gateway")
		expect(mockSetApiConfigurationField).toHaveBeenCalledWith("broGatewayModelId", broGatewayDefaultModelId, false)
	})
