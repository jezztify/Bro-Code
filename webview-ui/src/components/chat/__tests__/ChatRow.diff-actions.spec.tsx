	...
	useTranslation: () => ({
		t: (key: string) => {
			const map: Record<string, string> = {
				"chat:fileOperations.wantsToEdit": "Bro wants to edit this file",
				"chat:fileOperations.wantsToEditProtected": "Bro wants to edit a protected file",
				"chat:fileOperations.wantsToEditOutsideWorkspace": "Bro wants to edit outside workspace",
				"chat:fileOperations.wantsToApplyBatchChanges": "Bro wants to apply batch changes",
			}
			return map[key] || key
		},
	}),
...
describe("ChatRow - inline diff stats and actions", () => {
	...
	it("uses appliedDiff edit treatment (header/icon/diff stats)", () => {
		...
		expect(screen.getByText("Bro wants to edit this file")).toBeInTheDocument()
	...
	it("preserves protected and outside-workspace messaging in unified branch", () => {
		...
		expect(screen.getByText("Bro wants to edit outside workspace")).toBeInTheDocument()
		...
		expect(screen.getByText("Bro wants to edit a protected file")).toBeInTheDocument()
	...
	it("keeps batch diff handling for unified edit tools", () => {
		...
		expect(screen.getByText("Bro wants to apply batch changes")).toBeInTheDocument()
