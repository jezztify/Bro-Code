		...
		t: (key: string, options?: { version?: string }) => {
			const translations: Record<string, string> = {
				...
				"chat:announcement.release.highlight1":
					"Xiaomi MiMo provider: Added Xiaomi MiMo as a first-class API provider so you can configure MiMo models directly in Bro Code.",
			...
			if (key === "chat:announcement.title" || key === "chat:announcement.finalRelease.title") {
				return `Bro Code ${options?.version ?? ""} Released`
			}
...
describe("Announcement", () => {
	it("renders the announcement title and highlights", () => {
		...
		expect(screen.getByText("Bro Code 3.55.0 Released")).toBeInTheDocument()
		expect(
			screen.getByText(
				"Xiaomi MiMo provider: Added Xiaomi MiMo as a first-class API provider so you can configure MiMo models directly in Bro Code.",
			),
