/**
 * ExtensionHost - Loads and runs the Bro Code extension in CLI mode
 *
...
import { createVSCodeAPI, IExtensionHost, ExtensionHostEventMap, setRuntimeConfigValues } from "@bro-code/vscode-shim"
import { DebugLogger, setDebugLogEnabled } from "@bro-code/core/cli"
...
// When running from a release tarball, BRO_CLI_ROOT is set by the wrapper script.
...
const CLI_PACKAGE_ROOT = process.env.BRO_CLI_ROOT || findCliPackageRoot()
...
export interface ExtensionHostInterface extends IExtensionHost<ExtensionHostEventMap> {
	...
	runTask(prompt: string, taskId?: string, configuration?: BroCodeSettings, images?: string[]): Promise<void>
...
export class ExtensionHost extends EventEmitter implements ExtensionHostInterface {
	...
	private initialSettings: BroCodeSettings
	...
	constructor(options: ExtensionHostOptions) {
		...
		this.previousCliRuntimeEnv = process.env.BRO_CLI_RUNTIME
		...
		const baseSettings: BroCodeSettings = {
	...
	public markWebviewReady(): void {
		...
		setRuntimeConfigValues("bro-code", this.initialSettings as Record<string, unknown>)
	...
	async dispose(): Promise<void> {
		...
		if (this.previousCliRuntimeEnv === undefined) {
			...
			process.env.BRO_CLI_RUNTIME = this.previousCliRuntimeEnv
		}
	}
}
