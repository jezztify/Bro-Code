...
const RELEASES_URL = "https://api.github.com/repos/BroCodeInc/Bro-Code/releases?per_page=100"
...
export function runUpgradeInstaller(version?: string, spawnImpl: typeof spawn = spawn): Promise<void> {
	return new Promise((resolve, reject) => {
		const env = version ? { ...process.env, BRO_VERSION: version } : process.env
...
export async function upgrade(options: UpgradeOptions = {}): Promise<void> {
	...
	if (compareVersions(latestVersion, currentVersion) <= 0) {
		console.log("Bro CLI is already up to date.")
		return
	}

	console.log(`Upgrading Bro CLI from ${currentVersion} to ${latestVersion}...`)
	if (runInstaller) {
		await runInstaller()
	} else {
