...
This document explains how the Bro Code CLI detects and tracks the agent loop state.
...
const client = new ExtensionClient({
	...
	debug: true, // Writes to ~/.bro/cli-debug.log
})
...
The CLI uses type guards from `@bro-code/types` for categorization:

```typescript
import { isIdleAsk, isInteractiveAsk, isResumableAsk, isNonBlockingAsk } from "@bro-code/types"
...
Enable with `-d` flag. Logs go to `~/.bro/cli-debug.log`:

```bash
bro -d -P "Build something" --no-tui
```

View logs:

```bash
tail -f ~/.bro/cli-debug.log
```
