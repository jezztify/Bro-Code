#!/bin/bash
# Bro Code CLI Local Build Script
#
# Usage:
...
create_tarball() {
    ...
    RELEASE_DIR="$REPO_ROOT/bro-cli-${PLATFORM}"
    ...
    cat > "$RELEASE_DIR/bin/bro" << 'WRAPPER_EOF'
#!/usr/bin/env node
...
process.env.BRO_CLI_ROOT = join(__dirname, '..');
process.env.BRO_EXTENSION_PATH = join(__dirname, '..', 'extension');
const ripgrepPath = join(__dirname, 'rg');
if (existsSync(ripgrepPath)) {
  process.env.BRO_RIPGREP_PATH = ripgrepPath;
}
...
verify_local_install() {
    ...
    BRO_LOCAL_TARBALL="$TARBALL_PATH" \
    BRO_INSTALL_DIR="$VERIFY_INSTALL_DIR" \
    BRO_BIN_DIR="$VERIFY_BIN_DIR" \
    BRO_VERSION="$VERSION" \
    "$CLI_DIR/install.sh" || {
        rm -rf "$VERIFY_DIR"
    ...
    # Test --help
    if ! "$VERIFY_BIN_DIR/bro" --help > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
    ...
    # Test --version
    if ! "$VERIFY_BIN_DIR/bro" --version > /dev/null 2>&1; then
        rm -rf "$VERIFY_DIR"
...
print_summary() {
    ...
    if [ "$LOCAL_INSTALL" = true ]; then
        echo "  Installed to: ~/.bro/cli"
        echo "  Binary: ~/.local/bin/bro"
        echo ""
        echo "  Test it out:"
        echo "    bro --version"
        echo "    bro --help"
        ...
        echo "    BRO_LOCAL_TARBALL=$REPO_ROOT/$TARBALL ./apps/cli/install.sh"
        echo ""
