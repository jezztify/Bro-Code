#!/bin/sh
# Bro Code CLI Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/BroCodeInc/Bro-Code/main/apps/cli/install.sh | sh
#
# Environment variables:
#   BRO_INSTALL_DIR   - Installation directory (default: ~/.bro/cli)
#   BRO_BIN_DIR       - Binary symlink directory (default: ~/.local/bin)
#   BRO_VERSION       - Specific version to install (default: latest)
#   BRO_LOCAL_TARBALL - Path to local tarball to install (skips download)

set -e

# Configuration
INSTALL_DIR="${BRO_INSTALL_DIR:-$HOME/.bro/cli}"
BIN_DIR="${BRO_BIN_DIR:-$HOME/.local/bin}"
REPO="BroCodeInc/Bro-Code"
MIN_NODE_VERSION=20
...
get_version() {
    ...
    if [ -n "$BRO_LOCAL_TARBALL" ]; then
        VERSION="${BRO_VERSION:-local}"
        info "Using local tarball (version: $VERSION)"
        return
    fi
    
    if [ -n "$BRO_VERSION" ]; then
        VERSION="$BRO_VERSION"
...
download_and_install() {
    TARBALL="bro-cli-${PLATFORM}.tar.gz"
    ...
    if [ -n "$BRO_LOCAL_TARBALL" ]; then
        if [ ! -f "$BRO_LOCAL_TARBALL" ]; then
            error "Local tarball not found: $BRO_LOCAL_TARBALL"
        fi
        info "Using local tarball: $BRO_LOCAL_TARBALL"
        cp "$BRO_LOCAL_TARBALL" "$TMP_DIR/$TARBALL"
    else
...
setup_bin() {
    ...
    if [ -L "$BIN_DIR/bro" ] || [ -f "$BIN_DIR/bro" ]; then
        rm -f "$BIN_DIR/bro"
    fi
    
    ln -sf "$INSTALL_DIR/bin/bro" "$BIN_DIR/bro"
    info "Created symlink: $BIN_DIR/bro"
}
...
# Verify installation
verify_install() {
    if [ -x "$BIN_DIR/bro" ]; then
        info "Verifying installation..."
        # Just check if it runs without error
        "$BIN_DIR/bro" --version >/dev/null 2>&1 || true
    fi
}
...
print_success() {
    echo ""
    printf "${GREEN}${BOLD}✓ Bro Code CLI installed successfully!${NC}\n"
    echo ""
    echo "  Installation: $INSTALL_DIR"
    echo "  Binary: $BIN_DIR/bro"
    ...
    echo "    cd ~/my-project && bro \"What is this project?\""
    echo ""
}
