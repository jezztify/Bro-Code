---
description: "Prepare a new release of the Bro Code extension"
...
4. Summarize the changes. If the user did not specify a release type, ask whether this should be a major, minor, or patch release.
    ...
    - Bro Code nightlies should stay on `major.ODD_NUMBER.patch` and use a large patch number for CI-generated pre-releases.
...
5. Review and update the Marketplace-facing root `README.md`.
    ...
    - Check for stale upstream Bro Code wording that should now say Bro Code.
...
7. For a major or minor release:
    ...
    - Arrange translation updates for all supported locales affected by README, announcement, or package localization changes. Use the `/bro-translate` skill to propagate the updated `chat.json` announcement highlight keys and the "What's New" section to all supported locales.
