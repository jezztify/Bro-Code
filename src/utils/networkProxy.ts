 ...
 *
 * Provides proxy configuration for all outbound HTTP/HTTPS requests from the Bro Code extension.
...
async function configureUndiciProxy(config: ProxyConfig): Promise<void> {
	...
	try {
		...
		// To ensure Bro Code's `fetch()` calls are proxied, patch global fetch in debug mode.
