...
export async function getRipgrepDiagnostic(vscodeAppRoot: string): Promise<string> {
	...
	const lines: string[] = [
		`Bro Code Ripgrep Diagnostic (${new Date().toISOString()})`,
...
/**
 * Registers the `bro-code.showRipgrepDiagnostic` command. Thin wrapper —
...
export function registerRipgrepDiagnosticCommand(): vscode.Disposable {
	const channel = vscode.window.createOutputChannel("Bro Code Ripgrep Diagnostic")
	const command = vscode.commands.registerCommand(getCommand("showRipgrepDiagnostic"), async () => {
		...
		await vscode.window.showInformationMessage("Bro Code: ripgrep diagnostic copied to clipboard.")
	})
