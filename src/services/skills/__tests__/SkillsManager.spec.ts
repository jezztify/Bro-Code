...
// Global bro directory - computed once
const GLOBAL_BRO_DIR = p(HOME_DIR, ".bro")
const GLOBAL_AGENTS_DIR = p(HOME_DIR, ".agents")

// Mock bro-config
vi.mock("../../bro-config", () => ({
	getGlobalBroDirectory: () => GLOBAL_BRO_DIR,
...
describe("SkillsManager", () => {
	...
	const globalSkillsDir = p(GLOBAL_BRO_DIR, "skills")
	const globalSkillsCodeDir = p(GLOBAL_BRO_DIR, "skills-code")
	const globalSkillsArchitectDir = p(GLOBAL_BRO_DIR, "skills-architect")
	const projectBroDir = p(PROJECT_DIR, ".bro")
	const projectSkillsDir = p(projectBroDir, "skills")
...
# Too Long Description`)
		...
		it("should handle symlinked skills directory", async () => {
			...
			// Simulate .bro/skills being a symlink to /shared/skills
			mockDirectoryExists.mockImplementation(async (dir: string) => {
				return dir === globalSkillsDir
			})
...
Instructions here...`
		...
		it("should handle symlinked skill subdirectory", async () => {
			...
			// Simulate .bro/skills/my-alias being a symlink to /external/actual-skill
			mockDirectoryExists.mockImplementation(async (dir: string) => {
				return dir === globalSkillsDir
			})
...
Instructions here...`
		...
		it("should prioritize .bro skills over .agents skills with same name", async () => {
			const agentSkillDir = p(globalAgentsSkillsDir, "common-skill")
			const agentSkillMd = p(agentSkillDir, "SKILL.md")
			const broSkillDir = p(globalSkillsDir, "common-skill")
			const broSkillMd = p(broSkillDir, "SKILL.md")
			...
			mockStat.mockImplementation(async (pathArg: string) => {
				if (pathArg === agentSkillDir || pathArg === broSkillDir) {
					return { isDirectory: () => true }
				}
				throw new Error("Not found")
			})
			...
			mockFileExists.mockImplementation(async (file: string) => {
				return file === agentSkillMd || file === broSkillMd
			})
...
# Agent Common Skill`
				...
				if (file === broSkillMd) {
					return `---
name: common-skill
description: Bro version (should take priority)
---
...
# Bro Common Skill`
			...
			// .bro should override .agents
			expect(commonSkill?.description).toBe("Bro version (should take priority)")
		})
...
Instructions`)
	...
	describe("createSkill", () => {
		it("should create a new global skill", async () => {
			...
			expect(createdPath).toBe(p(GLOBAL_BRO_DIR, "skills", "new-skill", "SKILL.md"))
			expect(mockMkdir).toHaveBeenCalledWith(p(GLOBAL_BRO_DIR, "skills", "new-skill"), { recursive: true })
			expect(mockWriteFile).toHaveBeenCalled()

			// Verify the content written
			const writeCall = mockWriteFile.mock.calls[0]
			expect(writeCall[0]).toBe(p(GLOBAL_BRO_DIR, "skills", "new-skill", "SKILL.md"))
		...
		it("should create a mode-specific skill with modeSlugs array", async () => {
			...
			expect(createdPath).toBe(p(GLOBAL_BRO_DIR, "skills", "code-skill", "SKILL.md"))
		...
		it("should create a project skill", async () => {
			...
			expect(createdPath).toBe(p(PROJECT_DIR, ".bro", "skills", "project-skill", "SKILL.md"))
		})
...
Instructions`)
	...
	describe("moveSkill", () => {
		it("should move a skill from generic to mode-specific directory", async () => {
			...
			const destDir = p(GLOBAL_BRO_DIR, "skills-code", "test-skill")
			const destSkillsDir = p(GLOBAL_BRO_DIR, "skills-code")
...
Instructions`)
		...
		it("should move a skill from one mode to another", async () => {
			const sourceSkillsDir = p(GLOBAL_BRO_DIR, "skills-code")
			const sourceDir = p(sourceSkillsDir, "test-skill")
			const testSkillMd = p(sourceDir, "SKILL.md")
			const destDir = p(GLOBAL_BRO_DIR, "skills-architect", "test-skill")
			const destSkillsDir = p(GLOBAL_BRO_DIR, "skills-architect")
