...
import type { MarketplaceItem, MarketplaceItemType, InstallMarketplaceItemOptions, McpParameter } from "@bro-code/types"
...
export class SimpleInstaller {
	...
	private async installMode(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".bromodes" : "custom-modes.yaml"
				throw new Error(
	...
	private async installMcp(
		...
		try {
			...
			if (error.code === "ENOENT") {
				...
				const fileName = target === "project" ? ".bro/mcp.json" : "mcp-settings.json"
				throw new Error(
	...
	private async getModeFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".bromodes")
		} else {
	...
	private async getMcpFilePath(target: "project" | "global"): Promise<string> {
		if (target === "project") {
			...
			return path.join(workspaceFolder.uri.fsPath, ".bro", "mcp.json")
		} else {
			const globalSettingsPath = await ensureSettingsDirectoryExists(this.context)
