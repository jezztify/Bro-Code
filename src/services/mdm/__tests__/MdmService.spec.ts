...
vi.mock("@bro-code/cloud", () => ({
	...
	PRODUCTION_CLERK_BASE_URL: "https://clerk.brocode.com",
}))
	...
	t: vi.fn((key: string) => {
		const translations: Record<string, string> = {
			"mdm.errors.cloud_auth_required":
				"Your organization requires Bro Code Cloud authentication. Please sign in to continue.",
			"mdm.errors.organization_mismatch":
				"You must be authenticated with your organization's Bro Code Cloud account.",
...
import { CloudService, getClerkBaseUrl, PRODUCTION_CLERK_BASE_URL } from "@bro-code/cloud"
...
describe("MdmService", () => {
	...
	beforeEach(() => {
		...
		mockGetClerkBaseUrl.mockReturnValue("https://dev.clerk.brocode.com")
	...
	describe("platform-specific config paths", () => {
		...
		it("should use correct path for Windows in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "BroCode", "mdm.json"))
		})
		...
		it("should use correct path for Windows in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith(path.join("C:\\ProgramData", "BroCode", "mdm.dev.json"))
		})
		...
		it("should use correct path for macOS in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/BroCode/mdm.json")
		})
		...
		it("should use correct path for macOS in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/Library/Application Support/BroCode/mdm.dev.json")
		})
		...
		it("should use correct path for Linux in production", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/bro-code/mdm.json")
		})
		...
		it("should use correct path for Linux in development", async () => {
			...
			expect(mockFs.existsSync).toHaveBeenCalledWith("/etc/bro-code/mdm.dev.json")
		})
	...
	describe("compliance checking", () => {
		...
		it("should be non-compliant when not authenticated", async () => {
			...
			if (!compliance.compliant) {
				expect(compliance.reason).toContain("Your organization requires Bro Code Cloud authentication")
			}
		})
