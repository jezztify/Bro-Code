...
export class MdmService {
	...
	private getMdmConfigPath(): string {
		...
		switch (platform) {
			case "win32": {
				// Windows: %ProgramData%\BroCode\mdm.json or mdm.dev.json
				const programData = process.env.PROGRAMDATA || "C:\\ProgramData"
				return path.join(programData, "BroCode", configFileName)
			}

			case "darwin":
				// macOS: /Library/Application Support/BroCode/mdm.json or mdm.dev.json
				return `/Library/Application Support/BroCode/${configFileName}`

			case "linux":
			default:
				// Linux: /etc/bro-code/mdm.json or mdm.dev.json
				return `/etc/bro-code/${configFileName}`
		}
	}
