...
import type { ExperimentId } from "@bro-code/types"
...
export async function getEnvironmentDetails(cline: Task, includeFileDetails: boolean = false) {
	...
	// Filter paths through broIgnoreController
	const allowedVisibleFiles = cline.broIgnoreController
		? cline.broIgnoreController.filterPaths(visibleFilePaths)
	...
	// Filter paths through broIgnoreController
	const allowedOpenTabs = cline.broIgnoreController
		? cline.broIgnoreController.filterPaths(openTabPaths)
	...
	if (includeFileDetails) {
		...
		if (isDesktop) {
			...
			if (maxFiles === 0) {
				...
				const { showBroIgnoredFiles = false } = state ?? {}

				const result = formatResponse.formatFilesList(
					cline.cwd,
					files,
					didHitLimit,
					cline.broIgnoreController,
					showBroIgnoredFiles,
				)
