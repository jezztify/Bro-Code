...
/**
 * Verifies that a condense result actually fits within the allowed token budget. If the
 * projected token count (from condensing) is still over budget - e.g. the LLM's summary
 * was itself large - applies one additional aggressive truncation pass so the caller
 * never sends a request that's already known to be oversized.
 */
...
export async function manageContext({
	...
	// Fall back to sliding window truncation if needed. This also triggers when condensing
	// was attempted and failed while recovering from a *confirmed* API context-window-exceeded
	// error (forceTruncationOnCondenseFailure), even if the locally-estimated prevContextTokens
	// doesn't itself appear to exceed allowedTokens - the provider has already told us the
	// request didn't fit, so we must not return the conversation unchanged.
