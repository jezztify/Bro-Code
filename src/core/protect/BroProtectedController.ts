...
/**
 * Controls write access to Bro configuration files by enforcing protection patterns.
 * Prevents auto-approved modifications to sensitive Bro configuration files.
 */
export class BroProtectedController {
	private cwd: string
	private ignoreInstance: Ignore

	// Predefined list of protected Bro configuration patterns
	...
	constructor(cwd: string) {
		...
		this.ignoreInstance.add(BroProtectedController.PROTECTED_PATTERNS)
	}
	...
	getProtectionMessage(): string {
		return "This is a Bro configuration file and requires approval for modifications"
	}
	...
	getInstructions(): string {
		const patterns = BroProtectedController.PROTECTED_PATTERNS.join(", ")
		return `# Protected Files\n\n(The following Bro configuration file patterns are write-protected and always require approval for modifications, regardless of autoapproval settings. When using list_files, you'll notice a ${SHIELD_SYMBOL} next to files that are write-protected.)\n\nProtected patterns: ${patterns}`
	}
	...
	static getProtectedPatterns(): readonly string[] {
		return BroProtectedController.PROTECTED_PATTERNS
	}
}
