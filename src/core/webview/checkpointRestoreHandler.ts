...
export async function handleCheckpointRestoreOperation(config: CheckpointRestoreConfig): Promise<void> {
	...
	try {
		...
		if (operation === "delete" && currentCline && !currentCline.abort) {
			// The task is reinitialized in-place further down via createTaskWithHistoryItem()
			// with the same taskId, so keep its terminal alive for the new instance to reuse.
			currentCline.abortTask(false, { closeTerminals: false })
