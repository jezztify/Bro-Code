...
import { worktreeService, worktreeIncludeService, type CopyProgressCallback } from "@bro-code/core"
...
export async function handleSwitchWorktree(
	...
	try {
		...
		if (newWindow) {
			// Set the auto-open path so the new window opens Bro Code sidebar.
...
export async function handleGetWorktreeDefaults(provider: ClineProvider): Promise<WorktreeDefaultsResponse> {
	...
	const dotBroPath = path.join(os.homedir(), ".bro")
	const suggestedPath = path.join(dotBroPath, "worktrees", `${projectName}-${suffix}`)

	return {
		suggestedBranch: `worktree/bro-${suffix}`,
		suggestedPath,
	}
}
