...
describe("webviewMessageHandler - searchFiles with BroIgnore filtering", () => {
	...
	beforeEach(() => {
		...
		// Spy on the mock BroIgnoreController prototype methods
		mockFilterPaths = vi.fn()
		mockDispose = vi.fn()

		// Override the filterPaths method on the prototype
		;(BroIgnoreController.prototype as any).filterPaths = mockFilterPaths
		;(BroIgnoreController.prototype as any).initialize = vi.fn().mockResolvedValue(undefined)
		;(BroIgnoreController.prototype as any).dispose = mockDispose
	...
	it("should filter results using BroIgnoreController when showBroIgnoredFiles is false", async () => {
		...
		// Setup state with showBroIgnoredFiles = false
	...
	it("should not filter results when showBroIgnoredFiles is true", async () => {
		// Setup mock results from file search
		const mockResults = [
			{ path: "src/index.ts", type: "file" as const, label: "index.ts" },
			{ path: "secrets/config.json", type: "file" as const, label: "config.json" },
		...
		// Setup state with showBroIgnoredFiles = true
		;(mockClineProvider.getState as Mock).mockResolvedValue({
			showBroIgnoredFiles: true,
		})
	...
	it("should use existing BroIgnoreController from current task", async () => {
		// Setup mock results from file search
		const mockResults = [
			{ path: "src/index.ts", type: "file" as const, label: "index.ts" },
	...
	it("should default showBroIgnoredFiles to false when state is null", async () => {
		// Setup mock results from file search
		...
		// Verify filterPaths was called (showBroIgnoredFiles defaults to false)
		expect(mockFilterPaths).toHaveBeenCalled()
	})

	it("should dispose temporary BroIgnoreController after use", async () => {
		// Setup mock results from file search
		const mockResults = [{ path: "src/index.ts", type: "file" as const, label: "index.ts" }]
		mockSearchWorkspaceFiles.mockResolvedValue(mockResults)
