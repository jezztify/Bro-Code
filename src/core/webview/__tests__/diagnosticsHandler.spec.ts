...
describe("generateErrorDiagnostics", () => {
	...
	it("generates a diagnostics file with error metadata and history", async () => {
		...
		expect(result.filePath).toContain("bro-diagnostics-")
		...
		expect(String(writtenPath)).toContain("bro-diagnostics-test-tas")
		expect(String(writtenContent)).toContain(
			"// Please share this file with Bro Code Support (support@brocode.dev) to diagnose the issue faster",
		)
