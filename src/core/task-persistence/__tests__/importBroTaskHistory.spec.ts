...
import {
	importBroTaskHistory,
	isConcurrentDestinationClaimError,
	resolveBroHistoryImportPaths,
} from "../importBroTaskHistory"
...
describe("importBroTaskHistory", () => {
	...
	beforeEach(async () => {
		tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "bro-history-import-"))
		vi.clearAllMocks()
	})
	...
	it("resolves Roo and Bro storage roots from extension domains and configured custom paths", async () => {
		const broGlobalStoragePath = path.join(tempRoot, "globalStorage", "brocodeorganization.bro-code")
		const rooCustomStoragePath = path.join(tempRoot, "roo-custom")
		const broCustomStoragePath = path.join(tempRoot, "bro-custom")

		mockStorageConfiguration({
			roo: rooCustomStoragePath,
			bro: broCustomStoragePath,
		})

		const result = await resolveBroHistoryImportPaths(broGlobalStoragePath)

		expect(result.rooExtensionDomain).toBe("RooVeterinaryInc.roo-cline")
		expect(result.broExtensionDomain).toBe("BroCodeOrganization.bro-code")
		expect(result.rooStorageRoots).toEqual([
			path.join(tempRoot, "globalStorage", "rooveterinaryinc.roo-cline"),
			rooCustomStoragePath,
		])
		expect(result.broStorageRoot).toBe(broCustomStoragePath)
	})
	...
	it("falls back to the default Roo storage root when reading Roo custom storage fails", async () => {
		...
		expect(result.broStorageRoot).toBe(broGlobalStoragePath)
	})
	...
	it("copies Roo task directories into the active Bro storage root", async () => {
		const broGlobalStoragePath = path.join(tempRoot, "globalStorage", "brocodeorganization.bro-code")
		const rooDefaultStorageRoot = path.join(tempRoot, "globalStorage", "rooveterinaryinc.roo-cline")
		const rooCustomStorageRoot = path.join(tempRoot, "roo-custom")
		const broCustomStorageRoot = path.join(tempRoot, "bro-custom")

		mockStorageConfiguration({
			roo: rooCustomStorageRoot,
			bro: broCustomStorageRoot,
		})
		...
		const result = await importBroTaskHistory(broGlobalStoragePath)

		expect(result.foundTaskCount).toBe(2)
		expect(result.importedTaskCount).toBe(2)
		expect(result.importedFileCount).toBe(4)
		expect(
			await fs.readFile(path.join(broCustomStorageRoot, "tasks", "task-default", "ui_messages.json"), "utf8"),
		).toBe("default")
		expect(
			await fs.readFile(
				path.join(broCustomStorageRoot, "tasks", "task-custom", "api_conversation_history.json"),
				"utf8",
			),
		).toBe("custom")
		await expect(fs.access(path.join(broCustomStorageRoot, "tasks", "_index.json"))).rejects.toMatchObject({
			code: "ENOENT",
		})
	})

	it("does not overwrite an existing Bro task directory when the same Roo history is imported again", async () => {
		const broGlobalStoragePath = path.join(tempRoot, "globalStorage", "brocodeorganization.bro-code")
		const rooDefaultStorageRoot = path.join(tempRoot, "globalStorage", "rooveterinaryinc.roo-cline")
		...
		const firstImportResult = await importBroTaskHistory(broGlobalStoragePath)
		...
		const secondImportResult = await importBroTaskHistory(broGlobalStoragePath)
		...
		expect(
			await fs.readFile(path.join(broGlobalStoragePath, "tasks", "task-repeat", "history_item.json"), "utf8"),
		).toBe(makeHistoryItem("task-repeat", { source: "first-import" }))
		expect(
			await fs.readFile(path.join(broGlobalStoragePath, "tasks", "task-repeat", "ui_messages.json"), "utf8"),
		).toBe("first-ui")
	})
	...
	it("deterministically keeps the first importable Roo task when duplicate task IDs exist across roots", async () => {
		...
		expect(result.importedFileCount).toBe(4)
		expect(
			await fs.readFile(path.join(broGlobalStoragePath, "tasks", "task-shared", "history_item.json"), "utf8"),
		).toBe(makeHistoryItem("task-shared", { source: "default-root" }))
		expect(
			await fs.readFile(path.join(broGlobalStoragePath, "tasks", "task-shared", "ui_messages.json"), "utf8"),
		).toBe("default-ui")
		expect(
			await fs.readFile(
				path.join(broGlobalStoragePath, "tasks", "task-custom-only", "history_item.json"),
				"utf8",
			),
	...
	it("reports Roo history import progress as files are copied", async () => {
		...
		await importBroTaskHistory(broGlobalStoragePath, onProgress)
	...
	it("imports only top-level task history files and skips checkpoint directories", async () => {
		const broGlobalStoragePath = path.join(tempRoot, "globalStorage", "brocodeorganization.bro-code")
		const rooDefaultStorageRoot = path.join(tempRoot, "globalStorage", "rooveterinaryinc.roo-cline")
		const broCustomStorageRoot = path.join(tempRoot, "shared-storage")

		mockStorageConfiguration({
			roo: broCustomStorageRoot,
			bro: broCustomStorageRoot,
		})
		...
		expect(result.importedFileCount).toBe(4)
		expect(
			await fs.readFile(path.join(broCustomStorageRoot, "tasks", "task-visible", "ui_messages.json"), "utf8"),
		).toBe("visible-ui")
		expect(
			await fs.readFile(
				path.join(broCustomStorageRoot, "tasks", "task-visible", "api_conversation_history.json"),
				"utf8",
			),
		).toBe("visible-api")
		expect(
			await fs.readFile(path.join(broCustomStorageRoot, "tasks", "task-visible", "task_metadata.json"), "utf8"),
		).toBe("metadata")
		await expect(fs.access(path.join(broCustomStorageRoot, "tasks", ".task-hidden"))).rejects.toMatchObject({
			code: "ENOENT",
		})
		await expect(fs.access(path.join(broCustomStorageRoot, "tasks", "_task-hidden"))).rejects.toMatchObject({
			code: "ENOENT",
		})
		await expect(
			fs.access(
				path.join(broCustomStorageRoot, "tasks", "task-visible", "checkpoints", ".git", "objects", "object"),
			),
		).rejects.toMatchObject({
			code: "ENOENT",
		})
		await expect(fs.access(path.join(broCustomStorageRoot, "tasks", "loose.json"))).rejects.toMatchObject({
			code: "ENOENT",
		})
	})
	...
	it("ignores missing Roo task roots while still importing from available roots", async () => {
		...
		expect(
			await fs.readFile(path.join(broGlobalStoragePath, "tasks", "task-default", "history_item.json"), "utf8"),
	...
	it("skips tasks that do not have an importable history_item.json", async () => {
		...
		await expect(fs.access(path.join(broGlobalStoragePath, "tasks", "task-missing-history"))).rejects.toMatchObject(
			{
				code: "ENOENT",
			},
		)
	...
	it("does not delete an existing Bro task when the Roo task is missing history_item.json", async () => {
		const broGlobalStoragePath = path.join(tempRoot, "globalStorage", "brocodeorganization.bro-code")
		const rooDefaultStorageRoot = path.join(tempRoot, "globalStorage", "rooveterinaryinc.roo-cline")
		const existingBroTaskDirectory = path.join(broGlobalStoragePath, "tasks", "task-existing")
		...
		await fs.mkdir(existingBroTaskDirectory, { recursive: true })
		await fs.writeFile(path.join(existingBroTaskDirectory, "history_item.json"), "existing")

		const result = await importBroTaskHistory(broGlobalStoragePath)

		expect(result.importedTaskCount).toBe(0)
		expect(result.importedFileCount).toBe(0)
		expect(await fs.readFile(path.join(existingBroTaskDirectory, "history_item.json"), "utf8")).toBe("existing")
	})

	it("does not overwrite an existing Bro task when the Roo task is otherwise importable", async () => {
		const broGlobalStoragePath = path.join(tempRoot, "globalStorage", "brocodeorganization.bro-code")
		const rooDefaultStorageRoot = path.join(tempRoot, "globalStorage", "rooveterinaryinc.roo-cline")
		const existingBroTaskDirectory = path.join(broGlobalStoragePath, "tasks", "task-existing")
		...
		await fs.mkdir(existingBroTaskDirectory, { recursive: true })
		await fs.writeFile(path.join(existingBroTaskDirectory, "history_item.json"), "existing")
		await fs.writeFile(path.join(existingBroTaskDirectory, "ui_messages.json"), "existing-ui")
		...
		expect(result.importedFileCount).toBe(0)
		expect(await fs.readFile(path.join(existingBroTaskDirectory, "history_item.json"), "utf8")).toBe("existing")
		expect(await fs.readFile(path.join(existingBroTaskDirectory, "ui_messages.json"), "utf8")).toBe("existing-ui")
	})
	...
	it("rejects task IDs containing dots or underscore prefixes to prevent traversal", async () => {
		...
		expect(result.importedTaskCount).toBe(1)
		for (const name of unsafeCandidates) {
			await expect(fs.access(path.join(broGlobalStoragePath, "tasks", name))).rejects.toMatchObject({
				code: "ENOENT",
			})
		}
	})
	...
	it("rejects tasks whose history_item.json id field does not match the directory name or contains unsafe characters", async () => {
		...
		expect(result.importedTaskCount).toBe(1)
		await expect(fs.access(path.join(broGlobalStoragePath, "tasks", "task-mismatch"))).rejects.toMatchObject({
			code: "ENOENT",
		})
		await expect(fs.access(path.join(broGlobalStoragePath, "tasks", "task-traversal"))).rejects.toMatchObject({
			code: "ENOENT",
		})
		await expect(fs.access(path.join(broGlobalStoragePath, "tasks", "task-invalid-schema"))).rejects.toMatchObject({
			code: "ENOENT",
		})
		await expect(fs.access(path.join(broGlobalStoragePath, "tasks", "task-valid"))).resolves.toBeUndefined()
	})
	...
	it("rethrows unexpected task-root errors while importing Roo history", async () => {
		...
		await expect(importBroTaskHistory(broGlobalStoragePath)).rejects.toMatchObject({
			code: "ENOTDIR",
		})
	})
	...
	it("imports the complete file set when two calls run concurrently against the same Roo task", async () => {
		...
		const [result1, result2] = await Promise.all([
			importBroTaskHistory(broGlobalStoragePath),
			importBroTaskHistory(broGlobalStoragePath),
		])
		...
		const destTaskDir = path.join(broGlobalStoragePath, "tasks", "task-concurrent")
		...
		const tasksEntries = await fs.readdir(path.join(broGlobalStoragePath, "tasks"))
