...
import { historyItemSchema } from "@bro-code/types"

import { GlobalFileNames } from "../../shared/globalFileNames"
import { Package } from "../../shared/package"
import { getStorageBasePath } from "../../utils/storage"

// The legacy Roo Code extension is a separate, real third-party product we
// import history from — its extension identity is fixed and must not be
// rebranded along with this extension's own name.
...
// Valid task IDs are Unix-millisecond timestamps (all digits), but we
...
export interface BroHistoryImportPaths {
	rooExtensionDomain: string
	broExtensionDomain: string
	rooStorageRoots: string[]
	broStorageRoot: string
}

export interface BroHistoryImportResult extends BroHistoryImportPaths {
	foundTaskCount: number
	importedTaskCount: number
	importedFileCount: number
}

export interface BroHistoryImportProgress {
...
export const resolveBroHistoryImportPaths = async (globalStoragePath: string): Promise<BroHistoryImportPaths> => {
	const broExtensionDomain = `${Package.publisher}.${Package.name}`
	const broStorageRoot = await getStorageBasePath(globalStoragePath)
...
export const importBroTaskHistory = async (
	globalStoragePath: string,
	onProgress?: (progress: BroHistoryImportProgress) => Promise<void> | void,
): Promise<BroHistoryImportResult> => {
	const paths = await resolveBroHistoryImportPaths(globalStoragePath)
	const destinationComparablePath = toComparablePath(paths.broStorageRoot)
	const sourceRoots = paths.rooStorageRoots.filter(
		(sourceRoot) => toComparablePath(sourceRoot) !== destinationComparablePath,
	)
	const destinationTasksRoot = path.join(paths.broStorageRoot, "tasks")
