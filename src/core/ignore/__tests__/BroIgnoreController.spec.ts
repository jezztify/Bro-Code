// npx vitest core/ignore/__tests__/BroIgnoreController.spec.ts

import type { Mock } from "vitest"

import { BroIgnoreController, LOCK_TEXT_SYMBOL } from "../BroIgnoreController"
...
describe("BroIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: BroIgnoreController
	...
	beforeEach(() => {
		...
		// Create controller
		controller = new BroIgnoreController(TEST_CWD)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .broignore exists
		 */
		it("should load .broignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .broignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".broignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".broignore"), "utf8")

			// Verify content was stored
			expect(controller.broIgnoreContent).toBe("node_modules\n.git\nsecrets.json")
		...
		/**
		 * Tests the controller behavior when .broignore doesn't exist
		 */
		it("should allow all access when .broignore doesn't exist", async () => {
			// Setup mocks to simulate missing .broignore file
			...
			expect(controller.broIgnoreContent).toBeUndefined()
		 ...
		 */
		it("should set up file watcher for .broignore changes", async () => {
		 ...
		 */
		it("should handle errors when loading .broignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .broignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .broignore exists
		 */
		it("should allow all access when no .broignore content", async () => {
			// Create a new controller with no .broignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new BroIgnoreController(TEST_CWD)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .broignore exists
		 */
		it("should allow all commands when no .broignore exists", async () => {
			// Create a new controller with no .broignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new BroIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .broignore
		 */
		it("should generate formatted instructions when .broignore exists", async () => {
			// Setup .broignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .broignore exists
		 */
		it("should return undefined when no .broignore exists", async () => {
			// Setup no .broignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .broignore is created
		 */
		it("should reload .broignore when file is created", async () => {
			// Setup initial state without .broignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .broignore
			controller = new BroIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .broignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .broignore is changed
		 */
		it("should reload .broignore when file is changed", async () => {
			// Setup initial state with .broignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .broignore is deleted
		 */
		it("should reset when .broignore is deleted", async () => {
			// Setup initial state with .broignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
