...
export interface FoldedFileContextOptions {
	...
	cwd: string
	/** Optional BroIgnoreController for file access validation */
...
export async function generateFoldedFileContext(
	...
	const { maxCharacters = 50000, cwd, broIgnoreController } = options
	...
	for (let i = 0; i < filePaths.length; i++) {
		...
		try {
			...
			const definitions = await parseSourceCodeDefinitionsForFile(absolutePath, broIgnoreController)
