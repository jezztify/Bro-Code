...
export class ApplyPatchTool extends BaseTool<"apply_patch"> {
	...
	private async handleUpdateFile(
		...
		if (change.movePath) {
			...
			const moveAccessAllowed = task.broIgnoreController?.validateAccess(change.movePath)
			if (!moveAccessAllowed) {
				await task.say("broignore_error", change.movePath)
				pushToolResult(formatResponse.broIgnoreError(change.movePath))
				await task.diffViewProvider.reset()
				return
			}

			// Check if destination path is write-protected
			const isMovePathWriteProtected = task.broProtectedController?.isWriteProtected(change.movePath) || false
			...
			await task.fileContextTracker.trackFileContext(change.movePath, "bro_edited" as RecordSource)
		} else {
