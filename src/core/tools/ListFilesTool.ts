...
import { type ClineSayTool } from "@bro-code/types"
...
export class ListFilesTool extends BaseTool<"list_files"> {
	...
	async execute(params: ListFilesParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const { showBroIgnoredFiles = false } = (await task.providerRef.deref()?.getState()) ?? {}

			const result = formatResponse.formatFilesList(
				absolutePath,
				files,
				didHitLimit,
				task.broIgnoreController,
				showBroIgnoredFiles,
				task.broProtectedController,
			)
