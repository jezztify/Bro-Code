...
export class GenerateImageTool extends BaseTool<"generate_image"> {
	...
	async execute(params: GenerateImageParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		if (inputImagePath) {
			...
			const inputImageAccessAllowed = task.broIgnoreController?.validateAccess(inputImagePath)
			if (!inputImageAccessAllowed) {
				await task.say("broignore_error", inputImagePath)
				pushToolResult(formatResponse.broIgnoreError(inputImagePath))
				return
			}
		...
		try {
			...
			if (finalPath) {
				await task.fileContextTracker.trackFileContext(finalPath, "bro_edited")
			}
