...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn(function (key, options) {
		...
		if (key === "tools:toolRepetitionLimitReached" && options?.toolName) {
			return `Bro appears to be stuck in a loop, attempting the same action (${options.toolName}) repeatedly. This might indicate a problem with its current strategy.`
		}
		return key
	}),
}))
