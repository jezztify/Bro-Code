// npx vitest core/prompts/__tests__/responses-broignore.spec.ts
...
describe("BroIgnore Response Formatting", () => {
	...
	describe("formatResponse.broIgnoreError", () => {
		/**
		 * Tests the error message format for ignored files
		 */
		it("should format error message for ignored files", () => {
			const errorMessage = formatResponse.broIgnoreError("secrets/api-keys.json")
			...
			expect(parsed.suggestion).toContain("update the .broignore file")
		})
		...
		it("should include the file path in the error message", () => {
			...
			for (const testPath of paths) {
				const errorMessage = formatResponse.broIgnoreError(testPath)
				const parsed = JSON.parse(errorMessage) as any
	...
	describe("formatResponse.formatFilesList with BroIgnoreController", () => {
		/**
		 * Tests file listing with broignore controller
		 */
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new BroIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList when showBroIgnoredFiles is set to false
		 */
		it("should hide ignored files when showBroIgnoredFiles is false", async () => {
			// Create controller
			const controller = new BroIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Format with controller and showBroIgnoredFiles = false
			const result = formatResponse.formatFilesList(
				TEST_CWD,
				files,
				false,
				controller as any,
				false, // showBroIgnoredFiles = false
			)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with BroIgnoreController
		 */
		it("should handle truncation with BroIgnoreController", async () => {
			// Create controller
			const controller = new BroIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with BroIgnoreController", async () => {
			// Create controller
			const controller = new BroIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .broignore instructions for the LLM", async () => {
			// Create controller
			const controller = new BroIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .broignore exists", async () => {
			...
			// Create controller without .broignore
			const controller = new BroIgnoreController(TEST_CWD)
			await controller.initialize()
