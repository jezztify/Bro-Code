...
export async function parseMentions(
	...
	showBroIgnoredFiles: boolean = false,
...
async function getFileOrFolderContentWithMetadata(
	...
	try {
		...
		if (stats.isFile()) {
			...
			if (broIgnoreController && !broIgnoreController.validateAccess(unescapedPath)) {
				return {
					type: "file",
					path: mentionPath,
					content: `[read_file for '${mentionPath}']\nNote: File is ignored by .broignore.`,
				}
			}
			try {
			...
			for (let index = 0; index < entries.length; index++) {
				...
				if (broIgnoreController) {
					isIgnored = !broIgnoreController.validateAccess(entryPath)
				}

				if (isIgnored && !showBroIgnoredFiles) {
					continue
				}
