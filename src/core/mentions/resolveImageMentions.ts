...
export interface ResolveImageMentionsOptions {
	...
	images?: string[]
	cwd: string
	broIgnoreController?: { validateAccess: (filePath: string) => boolean }
 ...
 * - Respects `.broignore` via `broIgnoreController.validateAccess` when provided
 */
export async function resolveImageMentions({
	...
	for (const mention of imageMentions) {
		...
		if (broIgnoreController && !broIgnoreController.validateAccess(relPath)) {
			continue
		}
