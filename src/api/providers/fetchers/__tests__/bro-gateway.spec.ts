// npx vitest run src/api/providers/fetchers/__tests__/bro-gateway.spec.ts

import axios from "axios"

import { getBroGatewayModels, parseBroGatewayModel } from "../bro-gateway"

vitest.mock("axios")
vitest.mock("../../../../services/bro-code-auth", () => ({
	getCachedBroCodeToken: vitest.fn(function () {
		return ""
	}),
	getBroCodeBaseUrl: vitest.fn(function () {
		return "https://example.test"
	}),
	resolveBroGatewaySessionToken: vitest.fn(function (profileToken?: string) {
		return profileToken || undefined
	}),
}))
const mockedAxios = axios as any

describe("Bro Gateway Fetchers", () => {
	beforeEach(() => {
		vitest.clearAllMocks()
	})

	describe("getBroGatewayModels", () => {
		const baseUrl = "https://example.test/api/gateway/v1"
		const token = "bro_ext_test_token"
		...
		it("forwards the bearer token and timeout, filters non-language models", async () => {
			...
			const models = await getBroGatewayModels({
				broGatewayBaseUrl: baseUrl,
		...
		it("skips the request and returns {} when no token is available", async () => {
			const models = await getBroGatewayModels({ broGatewayBaseUrl: baseUrl } as any)
	...
	describe("parseBroGatewayModel", () => {
		it("delegates to the vercel-ai-gateway parser", () => {
			const result = parseBroGatewayModel({
				id: "anthropic/claude-sonnet-4",
				model: {
					id: "anthropic/claude-sonnet-4",
					object: "model",
