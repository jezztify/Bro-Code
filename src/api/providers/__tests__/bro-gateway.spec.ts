// npx vitest run src/api/providers/__tests__/bro-gateway.spec.ts
...
import { broGatewayDefaultModelId, BRO_GATEWAY_DEFAULT_TEMPERATURE } from "@bro-code/types"

import { BroGatewayHandler, classifyGatewayApiError, toGatewayStreamError } from "../bro-gateway"
import { ApiHandlerOptions } from "../../../shared/api"
import { Package } from "../../../shared/package"
import { clearBroCodeToken } from "../../../services/bro-code-auth"
...
const mockGetCachedBroCodeToken = vitest.hoisted(() => vitest.fn<() => string | undefined>(() => undefined))
const mockSessionCleared = vitest.hoisted(() => ({ value: false }))

vitest.mock("../../../services/bro-code-auth", () => ({
	getBroCodeBaseUrl: vitest.fn(function () {
		return "https://www.brocode.dev"
	}),
	getCachedBroCodeToken: () => mockGetCachedBroCodeToken() ?? "",
	resolveBroGatewaySessionToken: (profileToken?: string) => {
		const cached = mockGetCachedBroCodeToken()
		if (cached) return cached
		if (mockSessionCleared.value) return undefined
		return profileToken
	},
	clearBroCodeToken: vitest.fn(async () => {
		mockSessionCleared.value = true
		mockGetCachedBroCodeToken.mockReturnValue(undefined)
	}),
}))
...
describe("BroGatewayHandler", () => {
	...
	async function drainCreateMessage(handler: BroGatewayHandler) {
	...
	describe("constructor", () => {
		it("allows construction without a session token (auth is enforced at request time)", () => {
			expect(() => new BroGatewayHandler({})).not.toThrow()
		...
		it("prefers the secret-storage cache over a persisted profile token", () => {
			mockGetCachedBroCodeToken.mockReturnValue("bro_ext_cached_token")

			new BroGatewayHandler({
				broSessionToken: "bro_ext_stale_profile_token",
				broGatewayModelId: mockOptions.broGatewayModelId,
			})

			expect(OpenAI).toHaveBeenCalledWith(
				expect.objectContaining({
					apiKey: "bro_ext_cached_token",
				}),
			)
		})

		it("initializes OpenAI with Bro enrichment headers and session token", () => {
			const handler = new BroGatewayHandler({
				...mockOptions,
				broGatewayBaseUrl: "https://staging.brocode.dev/api/gateway/v1",
			})

			expect(handler).toBeInstanceOf(BroGatewayHandler)
			expect(OpenAI).toHaveBeenCalledWith({
				baseURL: "https://staging.brocode.dev/api/gateway/v1",
				apiKey: mockOptions.broSessionToken,
		...
		it("defaults the gateway base URL from getBroCodeBaseUrl", () => {
			new BroGatewayHandler(mockOptions)

			expect(OpenAI).toHaveBeenCalledWith(
				expect.objectContaining({
					baseURL: "https://www.brocode.dev/api/gateway/v1",
				}),
			)
		})
	})
	...
	describe("fetchModel", () => {
		it("returns configured model info", async () => {
			const handler = new BroGatewayHandler(mockOptions)
			const result = await handler.fetchModel()

			expect(result.id).toBe(mockOptions.broGatewayModelId)
			expect(result.info.maxTokens).toBe(64000)
			expect(result.info.supportsPromptCache).toBe(true)
		})

		it("falls back to the default model when none is configured", async () => {
			const handler = new BroGatewayHandler({ broSessionToken: "bro_ext_test_token" })
			const result = await handler.fetchModel()

			expect(result.id).toBe(broGatewayDefaultModelId)
		})
	})
	...
	describe("createMessage", () => {
		it("requires authentication at request time when no session token is available", async () => {
			...
			await expect(drainCreateMessage(handler)).rejects.toThrow(
				"Bro Gateway requires authentication. Please sign in to Bro Code first.",
			)
		})
		...
		it("requires authentication at request time when no session token is available", async () => {
			...
			}).rejects.toThrow("Bro Gateway requires authentication. Please sign in to Bro Code first.")
		})
		...
		it("uses the default temperature when none is provided", async () => {
			...
			expect(mockCreate).toHaveBeenCalledWith(
				expect.objectContaining({
					temperature: BRO_GATEWAY_DEFAULT_TEMPERATURE,
				}),
				expect.any(Object),
			)
		})
		...
		it("adds cache breakpoints for supported models", async () => {
			...
			const handler = new BroGatewayHandler({
				...mockOptions,
				broGatewayModelId: "anthropic/claude-3.5-haiku",
			})
		...
		it("surfaces the add-credits prompt when an in-stream error carries a budget code", async () => {
			...
			expect(showErrorMessage).toHaveBeenCalledWith(
				"common:broAuth.errors.budget_exceeded",
				"common:broAuth.buttons.add_credits",
			)
		})
	})
	...
	describe("completePrompt", () => {
		...
		it("wraps errors with a Bro Gateway prefix", async () => {
			const handler = new BroGatewayHandler(mockOptions)
			...
			await expect(handler.completePrompt("Test")).rejects.toThrow(
				"Bro Gateway completion error: upstream failure",
			)
		})
	...
	describe("toGatewayStreamError", () => {
		...
		it("falls back to a default message and leaves status/code undefined", () => {
			...
			expect(error.message).toBe("Bro Gateway stream error")
	...
	describe("surfaceGatewayApiError", () => {
		it("clears the cached token and offers re-sign-in on 401", async () => {
			...
			showErrorMessage.mockResolvedValueOnce("common:broAuth.buttons.sign_in")

			await expect(drainCreateMessage(handler)).rejects.toThrow()
			expect(clearBroCodeToken).toHaveBeenCalledTimes(1)
			expect(showErrorMessage).toHaveBeenCalledWith(
				"common:broAuth.errors.session_expired",
				"common:broAuth.buttons.sign_in",
			)
		...
		it("prompts to add credits on 402", async () => {
			...
			showErrorMessage.mockResolvedValueOnce("common:broAuth.buttons.add_credits")

			await expect(drainCreateMessage(handler)).rejects.toThrow()
			expect(clearBroCodeToken).not.toHaveBeenCalled()
			expect(showErrorMessage).toHaveBeenCalledWith(
				"common:broAuth.errors.out_of_credits",
				"common:broAuth.buttons.add_credits",
			)
			expect(openExternal).toHaveBeenCalledTimes(1)
		})
		...
		it("offers contact support on 403", async () => {
			...
			showErrorMessage.mockResolvedValueOnce("common:broAuth.buttons.contact_support")

			await expect(drainCreateMessage(handler)).rejects.toThrow()
			expect(showErrorMessage).toHaveBeenCalledWith(
				"common:broAuth.errors.account_unavailable",
				"common:broAuth.buttons.contact_support",
			)
			expect(openExternal).toHaveBeenCalledTimes(1)
		})
		...
		it("surfaces the gateway error then wraps the message in completePrompt", async () => {
			...
			await expect(handler.completePrompt("ping")).rejects.toThrow("Bro Gateway completion error: out of credits")
			expect(showErrorMessage).toHaveBeenCalledWith(
				"common:broAuth.errors.out_of_credits",
				"common:broAuth.buttons.add_credits",
			)
