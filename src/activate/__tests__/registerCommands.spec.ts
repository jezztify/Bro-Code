...
describe("getVisibleProviderOrLog", () => {
	...
	it("logs and returns undefined if no provider found", () => {
		...
		expect(mockOutputChannel.appendLine).toHaveBeenCalledWith("Cannot find any visible Bro Code instances.")
	})
})
...
describe("registerCommands handlers", () => {
	...
	it("settingsButtonClicked posts both settingsButtonClicked and didBecomeVisible actions", () => {
		handlers["bro-code.settingsButtonClicked"]()
	...
	it("historyButtonClicked posts historyButtonClicked action", () => {
		handlers["bro-code.historyButtonClicked"]()
	...
	it("marketplaceButtonClicked posts marketplaceButtonClicked action", () => {
		handlers["bro-code.marketplaceButtonClicked"]()
	...
	it("toggleAutoApprove awaits postMessage with toggleAutoApprove action", async () => {
		...
		const handlerPromise = handlers["bro-code.toggleAutoApprove"]() as Promise<unknown>
		let settled = false
	...
	it("focusInput awaits postMessage on the registered provider when a sidebar panel is active", async () => {
		...
		const handlerPromise = handlers["bro-code.focusInput"]() as Promise<unknown>
		let settled = false
		void handlerPromise.then(() => {
			settled = true
		})
	...
	it("focusInput does not post when no sidebar panel is active", async () => {
		await handlers["bro-code.focusInput"]()
	...
	it.each([
		{ command: "bro-code.settingsButtonClicked", prefix: "settingsButtonClicked", expectedCalls: 2 },
		{ command: "bro-code.historyButtonClicked", prefix: "historyButtonClicked", expectedCalls: 1 },
		{ command: "bro-code.marketplaceButtonClicked", prefix: "marketplaceButtonClicked", expectedCalls: 1 },
		{ command: "bro-code.acceptInput", prefix: "acceptInput", expectedCalls: 1 },
	])(
	...
	it("toggleAutoApprove logs to outputChannel when postMessageToWebview rejects", async () => {
		...
		await handlers["bro-code.toggleAutoApprove"]()
