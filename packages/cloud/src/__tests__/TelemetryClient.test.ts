...
import { type TelemetryPropertiesProvider, TelemetryEventName } from "@bro-code/types"
...
describe("TelemetryClient", () => {
	...
	describe("capture", () => {
		...
		it("should send request when event is capturable and validation passes", async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith(
				"https://app.brocode.com/api/events",
	...
	describe("backfillMessages", () => {
		...
		it("should send FormData request with correct structure when authenticated", async () => {
			...
			expect(mockFetch).toHaveBeenCalledWith(
				"https://app.brocode.com/api/events/backfill",
				expect.objectContaining({
					method: "POST",
					headers: {
