...
import type { SettingsService, AuthService } from "@bro-code/types"
...
vi.mock("../config", () => ({
	getBroCodeApiUrl: () => "https://app.brocode.com",
}))

vi.mock("../utils", () => ({
	getUserAgent: () => "Bro-Code 1.0.0",
}))
	...
	describe("shareTask", () => {
		it("should share task with organization visibility and copy to clipboard", async () => {
			const mockResponseData = {
				success: true,
				shareUrl: "https://app.brocode.com/share/abc123",
			}
			...
			expect(result.shareUrl).toBe("https://app.brocode.com/share/abc123")

			expect(mockFetch).toHaveBeenCalledWith("https://app.brocode.com/api/extension/share", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: "Bearer session-token",
					"User-Agent": "Bro-Code 1.0.0",
				},
				body: JSON.stringify({
					taskId: "task-123",
			...
			expect(vscode.env.clipboard.writeText).toHaveBeenCalledWith("https://app.brocode.com/share/abc123")
		})
