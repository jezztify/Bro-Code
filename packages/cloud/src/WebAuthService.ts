...
import { getClerkBaseUrl, getBroCodeApiUrl, PRODUCTION_CLERK_BASE_URL } from "./config.js"
...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			const publisher = packageJSON?.publisher ?? "BroVeterinaryInc"
			const name = packageJSON?.name ?? "bro-cline"
			...
			const url = landingPageSlug
				? `${getBroCodeApiUrl()}/l/${landingPageSlug}?${params.toString()}`
				: useProviderSignup
					? `${getBroCodeApiUrl()}/extension/provider-sign-up?${params.toString()}`
					: `${getBroCodeApiUrl()}/extension/sign-in?${params.toString()}`

			await vscode.env.openExternal(vscode.Uri.parse(url))
		} catch (error) {
			const context = landingPageSlug ? ` (landing page: ${landingPageSlug})` : ""
			this.log(`[auth] Error initiating Bro Code Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate Bro Code Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from Bro Code Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with Bro Code Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid Bro Code Cloud sign in url")
			}
		...
		try {
			...
			if (providerModel) {
				await this.context.globalState.update("bro-provider-model", providerModel)
				await this.context.globalState.update("bro-auth-skip-model", undefined)
				this.log(`[auth] Stored provider model: ${providerModel}`)
			} else {
				// No model was selected during signup - flag this for the webview
				await this.context.globalState.update("bro-auth-skip-model", true)
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with Bro Code Cloud")
			}

			this.log("[auth] Successfully authenticated with Bro Code Cloud")
		} catch (error) {
			this.log(`[auth] Error handling Bro Code Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle Bro Code Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from Bro Code Cloud")
			}

			this.log("[auth] Logged out from Bro Code Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from Bro Code Cloud: ${error}`)
			throw new Error(`Failed to log out from Bro Code Cloud: ${error}`)
		}
	}
