...
export class CloudService extends EventEmitter<CloudServiceEvents> implements Disposable {
	...
	public async initialize(): Promise<void> {
		...
		try {
			...
			// `pnpm --filter @bro-code-cloud/bromote-cli development auth job-token --job-id 1 --user-id user_2xmBhejNeDTwanM8CgIOnMgVxzC --org-id org_2wbhchVXZMQl8OS1yt0mrDazCpW`
			// The token will last for 1 hour.
			const cloudToken = process.env.BRO_CODE_CLOUD_TOKEN
			...
			const staticOrgSettings = process.env.BRO_CODE_CLOUD_ORG_SETTINGS
