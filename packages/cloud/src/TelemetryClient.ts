import {
	...
	TelemetryEventName,
	broCodeTelemetryEventSchema,
	TelemetryPropertiesProvider,
	TelemetryEventSubscription,
} from "@bro-code/types"

import { getBroCodeApiUrl } from "./config.js"
...
export class CloudTelemetryClient extends BaseTelemetryClient {
	...
	private async fetch(path: string, options: RequestInit, allowQueueing = true) {
		...
		const url = `${getBroCodeApiUrl()}/api/${path}`
	...
	public override async capture(event: TelemetryEvent) {
		...
		const result = broCodeTelemetryEventSchema.safeParse(payload)
	...
	public async backfillMessages(messages: ClineMessage[], taskId: string): Promise<void> {
		...
		try {
			...
			const url = `${getBroCodeApiUrl()}/api/events/backfill`
			const fetchOptions: RequestInit = {
				method: "POST",
				headers: {
					Authorization: `Bearer ${token}`,
	...
	public override isTelemetryEnabled(): boolean {
		if (process.env.BRO_CODE_DISABLE_TELEMETRY === "1") {
			return false
		}
