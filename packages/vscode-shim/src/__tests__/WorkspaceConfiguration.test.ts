...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("bro-cline", context)
			...
			setRuntimeConfig("bro-cline", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("bro-cline", "testSetting", "testValue")

			expect(getRuntimeConfig("bro-cline.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("bro-cline", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("bro-cline.setting1")).toBe("value1")
			expect(getRuntimeConfig("bro-cline.setting2")).toBe(42)
			expect(getRuntimeConfig("bro-cline.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("bro-cline.defined")).toBe("value")
			expect(getRuntimeConfig("bro-cline.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("bro-cline", "setting1", "value1")
			setRuntimeConfig("bro-cline", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("bro-cline.setting1")).toBeUndefined()
			expect(getRuntimeConfig("bro-cline.setting2")).toBeUndefined()
		})
