/**
 * Builds the Zod schema for .bromodes configuration files and converts it
 * to JSON Schema (draft-07). This module is the single source of truth for
 * both the generator script (scripts/generate-bromodes-schema.ts) and the
...
// Build the top-level .bromodes schema.
const bromodesZodSchema = z
	.object({
...
/**
 * Generates the JSON Schema object for .bromodes configuration files.
 * Includes metadata fields ($id, title, description).
 */
export function generateBromodesJsonSchema(): Record<string, unknown> {
	const jsonSchema = zodToJsonSchema(bromodesZodSchema, {
		$refStrategy: "none",
		target: "jsonSchema7",
	}) as Record<string, unknown>

	jsonSchema["$id"] = "https://github.com/BroCodeInc/Bro-Code/blob/main/schemas/bromodes.json"
	jsonSchema["title"] = "Bro Code Custom Modes"
	jsonSchema["description"] = "Schema for .bromodes configuration files used by Bro Code to define custom modes."
