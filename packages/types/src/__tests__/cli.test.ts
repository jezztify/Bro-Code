import {
	broCliControlEventSchema,
	broCliFinalOutputSchema,
	broCliInputCommandSchema,
	broCliStreamEventSchema,
} from "../cli.js"

describe("CLI types", () => {
	describe("broCliInputCommandSchema", () => {
		it("validates a start command", () => {
			const result = broCliInputCommandSchema.safeParse({
				command: "start",
				requestId: "req-1",
				prompt: "hello",
	...
	describe("broCliControlEventSchema", () => {
		it("validates a control done event", () => {
			const result = broCliControlEventSchema.safeParse({
				type: "control",
				subtype: "done",
				requestId: "req-3",
				command: "start",
	...
	describe("broCliStreamEventSchema", () => {
		it("accepts passthrough fields for forward compatibility", () => {
			const result = broCliStreamEventSchema.safeParse({
				type: "assistant",
				id: 42,
				content: "partial",
				customField: "future",
	...
	describe("broCliFinalOutputSchema", () => {
		it("validates final json output shape", () => {
			const result = broCliFinalOutputSchema.safeParse({
				type: "result",
				success: true,
				content: "done",
				events: [],
